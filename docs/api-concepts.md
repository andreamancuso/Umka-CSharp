...
Use `RequestInterrupt(message)` to ask the native runtime to stop at the next Umka interrupt check. The request may come from another managed thread while the owning thread is executing Umka code, or from a managed callback on the owning thread. An interrupted `Run()` or function call reports a normal `UmkaException` with Umka's interruption diagnostic and terminates the runtime. `IsInterruptRequested` reports a pending native interrupt request, and `ClearInterrupt()` clears a pending request before execution if the host decides not to cancel.
