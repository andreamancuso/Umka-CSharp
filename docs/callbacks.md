...
Callback results are validated against the Umka prototype before UmkaSharp writes to the native result slot. Use `frame.CanReturn(value)` inside the callback when generic host code needs to preflight a candidate `UmkaValue` against the active callback result metadata before returning it. Enum results use the same underlying integer rules as enum arguments; return them with `UmkaValue.FromEnum<TEnum>` or an explicit signed/unsigned integer value. Weak pointer results can be returned with `UmkaValue.FromWeakPointer(handle)`, where the handle is treated as an opaque 64-bit Umka value. Built-in `any` results can be returned with `UmkaAnyValue.Null.ToValue()`, scalar/string `UmkaAnyValue.From(...).ToValue()`, or `UmkaAnyValue.From(nativeValue).ToValue()` for an exact retained native payload from the same runtime. Fixed-layout static array, dynamic-array, `[]str`, nested dynamic-array, `[][]str`, struct, and map results can be returned with `UmkaValue.FromStaticArray<TElement>`, `UmkaValue.FromDynamicArray<TElement>`, `UmkaValue.FromDynamicArray(string?[])`, `UmkaValue.FromNestedDynamicArray<TElement>`, `UmkaValue.FromNestedDynamicArray(string?[][])`, `UmkaValue.FromStruct<T>`, `UmkaValue.FromMap<TKey, TValue>`, `UmkaValue.FromStringKeyMap<TValue>`, `UmkaValue.FromStringValueMap<TKey>`, or `UmkaValue.FromStringMap` when the managed size, element size, row lengths, array length, and map key/value layout where applicable exactly match Umka and the Umka type has no unsupported managed-reference fields or elements; weak pointer fields, elements, keys, and values use `ulong`, and fixed-layout elements, keys, and values may be sequential structs. Retained native values can be returned with `UmkaValue.FromNativeValue(value)` when the retained value belongs to the same runtime and has an equivalent Umka result type. Declared non-empty interface results can be returned as exact retained interface values or retained concrete Umka structs when Umka can construct the method table; retained direct `fn`/closure values are also callable from C# with `UmkaNativeValue.AsCallable()`. Fiber result kinds are exposed through metadata but rejected before writing a managed result. Mismatched `UmkaValue` kinds, unsupported reference-bearing aggregate, dynamic-array element, nested dynamic-array inner element, or map key/value result types, and out-of-range narrow integer, enum, `char`, or finite `real32` results are captured as managed callback failures.
