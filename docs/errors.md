...
Managed callback exceptions are captured on the returned `UmkaCallback.LastException`, mirrored on `UmkaRuntime.LastCallbackException`, and attached as `UmkaException.InnerException` when the Umka call reports the callback failure. `TryRun(out exception)` returns that same `UmkaException` object instead of throwing it. The failed call terminates the runtime just like other Umka runtime errors: `IsAlive` returns `false`, and later execution or function lookup attempts are rejected. This includes exceptions thrown by user callback code, wrong callback frame readers, invalid callback argument indexes, callback result kind mismatches, unsupported map callback results, unsupported reference-bearing aggregate callback result types, and out-of-range narrow integer, `char`, or finite `real32` callback results.
...
UmkaSharp prevalidates supported scalar, weak pointer, fixed-layout aggregate, copy-based dynamic-array, string-array, nested dynamic-array, and copy-based map function result readers and callback frame readers. Dynamic arrays are supported when the element type contains no Umka-managed references and the managed element layout exactly matches the Umka element layout; `[]str` uses dedicated string-array readers and writers; nested dynamic arrays are supported when the inner element type contains no Umka-managed references or is direct `str`. Maps are copied out only for function results and callback arguments when key/value types are fixed-layout without Umka-managed references, direct `str`, or supported dynamic-array values with reference-free or direct-`str` elements. UmkaSharp still does not expose rooted first-class managed wrappers for long-lived dynamic arrays, maps, interfaces, fibers, or `any`, does not expose arbitrary Umka closures as C#-callable or retained objects, does not strengthen or validate weak pointer targets, and does not construct Umka maps from C# values.
